...
import { ContainerStyle, EmailStyle, PwStyle, LoginStyle, Signin, First, Neww, ImgStyle } from './style';
...
function LoginForm({ mutate }: LoginFormProps) {
  ...
  return (
    <ContainerStyle onSubmit={onSubmit} className="container">
      ...
      <LoginStyle type="submit" className="login">
        로그인
      </LoginStyle>
      <First>
        처음이신가요? <Neww onClick={toSignUp}>회원가입</Neww>
      </First>
    </ContainerStyle>
  );
}
